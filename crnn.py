...
def train(datapath, labelpath, epochs, batch_size, lr, finetune=False):
    ...
    kfold = list(KFold(nsplits, shuffle=True, random_state=2018).split(nfiles))
    for idx in range(nsplits):
        train_kfold(idx, kfold, datapath, labelpath, epochs, batch_size, lr, finetune)

if __name__=='__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--train", default='Challenge 1_ Handwriting OCR for Vietnamese Address/0916_Data Samples 2/', type=str)
    parser.add_argument("--label", default='Challenge 1_ Handwriting OCR for Vietnamese Address/labels.json', type=str)
